...
def ensure_schema():
    ...
    """)
    # Every other table the dashboard reads from — created here too, so the
    # dashboard works even before the bot has finished its own startup (or
    # if the bot is having trouble connecting to Discord). Column
    # definitions match main.py's init_db() exactly, so whichever side
    # creates the table first, the other's CREATE TABLE IF NOT EXISTS is a
    # no-op and nothing conflicts.
    conn.execute("""
...
def debug_oauth():
    """Visit this in a browser to see exactly what redirect_uri gets sent
    to Discord — paste that exact string into Discord's portal Redirects
    list if login keeps failing."""
    computed = f"{DASHBOARD_URL_BASE()}/callback"
    return {
        "DASHBOARD_URL env var (raw)": os.getenv("DASHBOARD_URL") or "(not set)",
        "computed redirect_uri": computed,
        "DISCORD_CLIENT_ID set": bool(DISCORD_CLIENT_ID),
        "note": "This exact 'computed redirect_uri' string must appear, character "
                "for character, in Discord's portal → OAuth2 → Redirects.",
    }
...
def DASHBOARD_URL_BASE():
    ...
    # No DASHBOARD_URL set — fall back to guessing from the request, but
    # respect the proxy's original scheme (Render/Heroku/etc terminate
    # HTTPS at their edge and forward plain HTTP internally, so
    # request.url_root alone would report http:// even on a live https
    # site, which Discord then rejects as a redirect_uri mismatch).
